...
-- STEP 1: Add launch_type column to node_tokens table
-- =====================================================
...
ON node_tokens(launch_type);

-- Add comment for documentation
COMMENT ON COLUMN node_tokens.launch_type IS 'Launch type: STANDARD (B2C 0.5%/0.5%) or AGENT_LAUNCH (B2B 0.75%/0.25%)';
COMMENT ON COLUMN node_tokens.fee_split IS 'Fee distribution: {creator: number, dewa: number}';
...
SELECT 
  ...
  -- Fee tracking (assuming node_tokens table has trading_fees column)
...
SET 
  standard_launches = (
    SELECT COUNT(*) FROM node_tokens t 
  ...
  total_fees_earned = (
    SELECT COALESCE(SUM(trading_fees), 0) FROM node_tokens t 
